[![Build Status](https://travis-ci.com/soluwalana/kafka-security-manager.svg?branch=master)](https://travis-ci.com/soluwalana/kafka-security-manager)
...
SNAPSHOTS artifacts are deployed to [Sonatype](https://oss.sonatype.org/content/repositories/snapshots/com/github/soluwalana/ksm/)

RELEASES artifacts are deployed to [Maven Central](http://search.maven.org/#search%7Cga%7C1%7Ccom.github.soluwalana):

`build.sbt` (see [Maven Central](http://search.maven.org/#search%7Cga%7C1%7Ccom.github.soluwalana) for the latest `version`)
```
libraryDependencies += "com.github.soluwalana" %% "kafka-security-manager" % "version"
```
...
When configured authorizer class is `com.github.soluwalana.ksm.compat.AdminClientAuthorizer`,
...
- `AUTHORIZER_CLASS`: authorizer class for ACL operations. Default is `SimpleAclAuthorizer`, configured with
  ...
  No-zookeeper authorizer class on top of Kafka Admin Client is bundled with KSM as `com.github.soluwalana.ksm.compat.AdminClientAuthorizer`,
...
- `SOURCE_CLASS`: Source class. Valid values include
    - `com.github.soluwalana.ksm.source.NoSourceAcl` (default): No source for the ACLs. Only use with `KSM_READONLY=true`
    - `com.github.soluwalana.ksm.source.FileSourceAcl`: get the ACL source from a file on disk. Good for POC
    - `com.github.soluwalana.ksm.source.GitHubSourceAcl`: get the ACL from GitHub. Great to get started quickly and store the ACL securely under version control.
    - `com.github.soluwalana.ksm.source.GitLabSourceAcl`: get the ACL from GitLab using pesonal access tokens. Great to get started quickly and store the ACL securely under version control.
    ...
    - `com.github.soluwalana.ksm.source.S3SourceAcl`: get the ACL from S3. Good for when you have a S3 bucket managed by Terraform or Cloudformation. This requires `region`, `bucketname` and `objectkey`. See [Access credentials](https://docs.aws.amazon.com/sdk-for-java/v1/developer-guide/credentials.html) for credentials management.
    ...
    - `com.github.soluwalana.ksm.source.BitbucketServerSourceAcl`: get the ACL from Bitbucket Server using the v1 REST API. Great if you have private repos in Bitbucket.
    - `com.github.soluwalana.ksm.source.BitbucketCloudSourceAcl`: get the ACL from Bitbucket Cloud using the Bitbucket Cloud REST API v2.
- `NOTIFICATION_CLASS`: Class for notification in case of ACL changes in Kafka.
    - `com.github.soluwalana.ksm.notification.ConsoleNotification` (default): Print changes to the console. Useful for logging
    - `com.github.soluwalana.ksm.notification.SlackNotification`: Send notifications to a Slack channel (useful for devops / admin team)
...
Alternatively, you can get the automatically built Docker images on [Docker Hub](https://hub.docker.com/r/soluwalana/kafka-security-manager)  
...
docker run -it -e AUTHORIZER_ZOOKEEPER_CONNECT="zookeeper-url:2181" -e KSM_EXTRACT=true \
            soluwalana/kafka-security-manager:latest
```
...
For full usage of the docker-compose file see [kafka-stack-docker-compose](https://github.com/soluwalana/kafka-stack-docker-compose)
